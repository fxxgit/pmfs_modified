...
#define _mm_clflush(addr) __flush_dcache_area(addr,CACHELINE_SIZE)
...
//add fxx			_mm_clwb(buf + i);
			_mm_clflush(buf + i);//add fxx end
	} else {
		for (i = 0; i < len; i += CACHELINE_SIZE)
			_mm_clflush(buf + i);
	}
